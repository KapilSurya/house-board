  ...
  return <section id="features" className="py-20 relative">
      ...
      <div className="container mx-auto px-4 pt-8">
        ...
        <div className="flex flex-col gap-14 max-w-6xl mx-auto">
          {features.map((feature, index) => <Card key={feature.id} className={`overflow-hidden transition-all duration-300 hover:shadow-md card-hover flex flex-col md:flex-row items-center ${index % 2 !== 0 ? 'md:flex-row' : 'md:flex-row-reverse'} p-6 rounded-3xl`}>
              <div className="md:w-1/2 flex justify-center px-6 py-8">
                <div className="relative w-full max-w-2xl rounded-md overflow-hidden">
                  {feature.mockupImage && <img src={feature.mockupImage} alt={feature.imageAlt || `${feature.title} illustration`} className="w-full h-auto object-contain" style={{
                maxHeight: '500px',
                width: '100%'
              }} />}

                  {/* Add candle light decoration for each feature */}
                  {index % 2 === 0 ? <div className="absolute bottom-2 right-2 candle-light" style={{
                width: '15px',
                height: '15px'
              }}></div> : <div className="absolute bottom-2 left-2 candle-light" style={{
                width: '15px',
                height: '15px'
              }}></div>}
                </div>
